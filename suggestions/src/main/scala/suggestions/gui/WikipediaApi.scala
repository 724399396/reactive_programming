...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    /**
      ...
      */
    def recovered: Observable[Try[T]] = obs.materialize.filter(n => n match {
    ...
    /**
      ...
      */
    def timedOut(totalSec: Long): Observable[T] = obs.takeUntil(Observable.interval(Duration(totalSec, SECONDS)))
    ...
    /** Given a stream of events `obs` and a method `requestMethod` to map a request `T` into
      ...
      */
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = obs.map(t => requestMethod(t)).map(s => s.recovered).concat
  }
